...
from .abbr import RGROUP_SYMBOLS, ABBREVIATIONS, VALENCES, FORMULA_REGEX,SUBSTITUTIONS
...
def extract_stereochemistry(smiles):
    """
    This function extracts the stereochemistry information from a SMILES string.
    It returns a list of tuples where each tuple contains the index of the chiral center and its stereochemistry.
    """
    stereochemistry = []
    tokens = smiles.split('[')
...
def _expand_abbreviation_(abbrev, threshold=0.8):
    ...
    for substitution in SUBSTITUTIONS:
        for known_abbrev in substitution.abbrvs:
            similarity = string_similarity(abbrev, known_abbrev)
...
def _verify_chirality(mol, coords, symbols, edges, debug=False):
    try:
        ...
        #Chem.FindPotentialStereo(mol, force=True)
...
def _expand_functional_group(mol, mappings, debug=False):
    ...
    print('mappings1',mappings)
    original_chiral_centers1 = Chem.FindMolChiralCenters(mol, includeUnassigned=True)
    print("Original chiral centers1 in _expand_functional_group: ", original_chiral_centers1)
    if _need_expand(mol, mappings):
        ...
        for i in range(num_atoms):
            ...
            if atom.GetSymbol() == '*' or atom.GetSymbol() == 'N':
                ...
                original_chiral_centers = Chem.FindMolChiralCenters(mol_w , includeUnassigned=True)
                print("Original chiral centers in _expand_functional_group: ", original_chiral_centers)
                ...
                middle_chiral_centers = Chem.FindMolChiralCenters(mol_w, includeUnassigned=True)
                print("middle chiral centers in _expand_functional_group: ", middle_chiral_centers)
        ...
        for i in atoms_to_remove:
            mol_w.RemoveAtom(i)
        original_stereochemistry = extract_stereochemistry(smiles1)

        smiles = Chem.MolToSmiles(mol_w)
        print('#################smlies2.5',smiles)
        total_stereo_count = len(re.findall(r'\[C@[H]?\]', smiles)) + len(re.findall(r'\[C@@[H]?\]', smiles))

        new_stereochemistry = extract_stereochemistry(smiles)
...
def _postprocess_smiles(smiles, coords=None, symbols=None, edges=None, molblock=False, debug=False):
    ...
    try:
        print('##################################################start postprocess_smiles')
        pred_smiles = smiles
