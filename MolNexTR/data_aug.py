...
from albumentations.augmentations.geometric.functional import safe_rotate_enlarged_img_size, _maybe_process_in_chunks, keypoint_rotate
import cv2
import math
import random
import numpy as np
...
def safe_rotate(
    ...
    """Safely rotates an image without cropping the corners."""
...
def keypoint_safe_rotate(keypoint, angle, rows, cols):
    """Safely rotates a keypoint by a specified angle while adjusting its position."""
    old_rows = rows
    old_cols = cols
    new_rows, new_cols = safe_rotate_enlarged_img_size(angle=angle, rows=old_rows, cols=old_cols)
...
class SafeRotate(A.SafeRotate):
    """Applies a safe rotation to images and keypoints while preserving corners."""
    def __init__(self, limit=90, interpolation=cv2.INTER_LINEAR, border_mode=cv2.BORDER_REFLECT_101, value=None, mask_value=None, always_apply=False, p=0.5):
        super(SafeRotate, self).__init__(limit=limit, interpolation=interpolation, border_mode=border_mode, value=value, mask_value=mask_value, always_apply=always_apply, p=p)

    def apply(self, img, angle=0, interpolation=cv2.INTER_LINEAR, **params):
        return safe_rotate(img=img, value=self.value, angle=angle, interpolation=interpolation, border_mode=self.border_mode)
...
class CropWhite(A.DualTransform):
    """Crops an image to remove white (or specified color) padding from the edges."""
    ...
    def update_params(self, params, **kwargs):
        """Updates cropping parameters to remove the specified padding color from the edges."""
        ...
        while col_sum[right-1] == 0 and right-1 > left:
            right -= 1
        params.update({"crop_top": top, "crop_bottom": height - bottom, "crop_left": left, "crop_right": width - right})
        return params
    ...
    def apply(self, img, crop_top=0, crop_bottom=0, crop_left=0, crop_right=0, **params):
        ...
        img = A.augmentations.pad_with_params(img, self.pad, self.pad, self.pad, self.pad, border_mode=cv2.BORDER_CONSTANT, value=self.value)
        return img

class PadWhite(A.DualTransform):
    """Randomly pads one of the image edges with a specified color to augment the image."""
    ...
    def update_params(self, params, **kwargs):
        """Sets random padding for one of the four sides."""
        super().update_params(params, **kwargs)
        assert "image" in kwargs
        img = kwargs["image"]
        height, width, _ = img.shape
...
class SaltAndPepperNoise(A.DualTransform):
    """Applies salt-and-pepper noise by randomly setting pixels to a specified color."""
...
class ResizePad(A.DualTransform):
    """Resizes and centers an image to fit within specified dimensions with padding if necessary."""
    ...
    def apply(self, img, interpolation=cv2.INTER_LINEAR, **params):
        h, w, _ = img.shape
        img = A.augmentations.geometric.functional.resize(img, height=min(h, self.height), width=min(w, self.width), interpolation=interpolation)
        h, w, _ = img.shape
        pad_top = (self.height - h) // 2
        pad_bottom = (self.height - h) - pad_top
        pad_left = (self.width - w) // 2
        pad_right = (self.width - w) - pad_left
        img = A.augmentations.pad_with_params(img, pad_top, pad_bottom, pad_left, pad_right, border_mode=cv2.BORDER_CONSTANT, value=self.value)
        return img

def normalized_grid_distortion(img, num_steps=10, xsteps=(), ysteps=(), *args, **kwargs):
    """Applies normalized grid distortion to an image with specified step sizes."""
...
class NormalizedGridDistortion(A.augmentations.transforms.GridDistortion):
    """A custom GridDistortion with normalized step sizes for bounded distortions."""
...
class PadToSquare(A.DualTransform):
    """Pads an image to make it square by adding the specified color on shorter sides."""
    ...
    def apply(self, img, **params):
        ...
        if height <= width:
            img = A.augmentations.pad_with_params(img, pad1, pad2, 0, 0, border_mode=cv2.BORDER_CONSTANT, value=self.value)
        else:
            img = A.augmentations.pad_with_params(img, 0, 0, pad1, pad2, border_mode=cv2.BORDER_CONSTANT, value=self.value)
        return img

class ConditionalPadToSquare(A.DualTransform):
    """Pads an image to make it square if its aspect ratio exceeds a specified threshold."""
...
class AddLineNoise(A.ImageOnlyTransform):
    """Draws random lines on the image to simulate noise."""
...
class AddEdgeElementSymbolNoise(A.ImageOnlyTransform):
    """Adds chemical element symbols at random positions along the image edges."""
    def __init__(self, num_symbols, color=(0, 0, 0), font_scale=1, thickness=3, edge_width=30, always_apply=False, p=1):
        ...
        self.elements = ["H", "He", "Li", "Be", "B", "C", "N", "O", "F", "Ne", "Na", "Mg", "Al", "Si", "P", "S", "Cl", "Ar", "K", "Ca",
                         ...
                         "Rg", "Cn", "Nh", "Fl", "Mc", "Lv", "Ts", "Og", 'R', 'R1', 'R2', 'R3', 'R4', 'R5', 'R6', 'R7', 'R8', 'R9', 
                         'R10', 'R11', 'R12', 'Ra', 'Rb', 'Rc', 'Rd', 'X', 'Y', 'Z', 'Q', 'A', 'E', 'Ar']
...
class DrawBorder(A.ImageOnlyTransform):
    """Draws a border around the non-white areas of the image."""
