...
class AddArrowNoise(A.ImageOnlyTransform):
    def __init__(self, num_arrows, dilation_radius=10, p=0.5):
        super().__init__(p)
    ...
    def apply(self, image, **params):
        h, w = image.shape[:2]
        white_areas = np.all(image == [255, 255, 255], axis=-1)
        dilated_white_areas = cv2.dilate(white_areas.astype(np.uint8), None, iterations=self.dilation_radius)
        noise_coords = np.argwhere(dilated_white_areas)

        for _ in range(self.num_arrows):
            if len(noise_coords) > 1:  # We need at least two points to draw an arrow
                start_x, start_y = noise_coords[np.random.randint(len(noise_coords))]
                end_x, end_y = noise_coords[np.random.randint(len(noise_coords))]
                # Calculate the distance between the start point and the end point
                distance = np.sqrt((end_x - start_x) ** 2 + (end_y - start_y) ** 2)
                # Only draw the arrow if the distance is less than 100
                if distance <= 30:
                    cv2.line(image, (start_y, start_x), (end_y, end_x), (0, 0, 0), 2)
...
class AddElementSymbolNoise(A.ImageOnlyTransform):
    def __init__(self, num_symbols, color=(0, 0, 0), font_scale=1, thickness=3, distance=30, always_apply=False, p=1):
        super().__init__(always_apply, p)
        self.num_symbols = num_symbols
        self.color = color
        self.font_scale = font_scale
        ...
        self.elements = ["H", "He", "Li", "Be", "B", "C", "N", "O", "F", "Ne",
                         ...
                         "Pa", "U", "Np", "Pu", "Am", "Cm", "Bk", "Cf", "Es", "Fm",
                         "Md", "No", "Lr", "Rf", "Db", "Sg", "Bh", "Hs", "Mt", "Ds",
                         "Rg", "Cn", "Nh", "Fl", "Mc", "Lv", "Ts", "Og", 'R', 'R1', 'R2', 'R3', 'R4', 'R5', 'R6', 'R7',
                         'R8', 'R9', 'R10', 'R11', 'R12',
                         'Ra', 'Rb', 'Rc', 'Rd', 'X', 'Y', 'Z', 'Q', 'A', 'E', 'Ar']  # A list of elements symbols.

    def apply(self, image, **params):
        h, w = image.shape[:2]
        white_areas = np.all(image == [255, 255, 255], axis=-1)
        dilated_white_areas = cv2.dilate(white_areas.astype(np.uint8), None, iterations=self.distance)
        noise_positions = dilated_white_areas > white_areas
        noise_coords = np.argwhere(noise_positions)

        for _ in range(self.num_symbols):
            if len(noise_coords) > 0:
                x, y = noise_coords[np.random.randint(len(noise_coords))]
                symbol = random.choice(self.elements)
                cv2.putText(image, symbol, (y, x), cv2.FONT_HERSHEY_SIMPLEX, self.font_scale, self.color,
                            self.thickness)
        return image
